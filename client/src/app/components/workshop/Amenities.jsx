...
const Amenities = () => {
  return (
    <section className="py-16 md:py-24 bg-custom-black overflow-hidden relative border-t border-white/10">
      {/* Decorative Circles */}
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Experience Comfort While You Wait</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
          {amenities.map((item, index) => (
            <div key={index} className="flex flex-col items-center p-6 bg-custom-jet rounded-lg border border-white/10 hover:shadow-md transition-all duration-300 group">
              <div className="text-3xl mb-4 text-workshop-red group-hover:scale-110 transition-transform">
                {item.icon}
              </div>
              <span className="font-semibold text-custom-platinum text-sm text-center">{item.label}</span>
            </div>
          ))}
        </div>
      </div>
