...
exports.generateTopics = async (req, res) => {
  try {
    ...
    // Handle case where AI returns { topics: [...] } or just the array directly if structured differently, 
    // but json_object mode usually requires asking for a specific schema or it infers. 
    // Let's adjust prompt to be safe or parse carefully.
    // Actually, let's refine the prompt to ask for a specific key.
  ...
  try {
    ...
    try {
      const imageResponse = await openai.images.generate({
        model: "dall-e-3",
        prompt: `A realistic, high-quality, professional photo for a blog post about: ${blogData.title}. The image should be suitable for a car dealership in India. No text in the image.`,
        n: 1,
        size: "1024x1024",
      });
      ...
      const filename = `blog-images/${timestamp}-${Math.random().toString(36).substring(7)}.png`; // DALL-E 3 returns PNG by default usually
      ...
      blogData.featuredImage = s3Url; // Attach to response
